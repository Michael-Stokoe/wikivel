    ...
    <div class="text-right w-1/2 h-full flex-2">
        <div class="w-full h-full flex">
            ...
            <div class="w-2/12 h-full text-right">
                <form class="flex-2" method="POST" action="{{ route('space.delete', ['space' => $space]) }}">
                    @csrf
                    ...
                    <a class="px-1 hover:text-yellow-600 hover:no-underline"
                        href="{{ route('space.edit', ['space' => $space]) }}">
...
@include('comments.partials.commentsection', ['comments' => $space->comments->sortByDesc('id')])
